document.addEventListener("DOMContentLoaded", () => {
    ...
    function fetchMessages(sidToken, seq = 0) {
        Swal.showLoading(); // Show loading while fetching messages
        axios.post('https://pyeulmail-serverapi-production.up.railway.app/check_messages', { sid_token: sidToken, seq })
            ...
            .finally(() => Swal.close()); // Close loading when done
    }
    ...
    // ** Function to Start Polling Messages every 15 seconds and stop after 1.5 minutes (90 seconds) **
    function startPolling(sidToken) {
        if (pollingInterval) {
            clearInterval(pollingInterval);
        }

        // Start fetching messages every 15 seconds
        pollingInterval = setInterval(() => fetchMessages(sidToken), 15000);

        // Stop polling after 1.5 minutes (90 seconds)
        pollingTimeout = setTimeout(() => {
            clearInterval(pollingInterval);
            Swal.fire('Polling Stopped', 'The inbox has been checked for 1.5 minutes. Polling has stopped.', 'info');
